...
def setup_memory_usage_file_if_enabled() -> None:
    if config.USE_MEMORY_PROFILING:
        print(f"Setting up memory profile output file: \
            {PROFILING_OUTPUT_FILE}")
        # Clear the file so each run is unique.
        with open(os.path.join(PROFILING_DIRECTORY, PROFILING_OUTPUT_FILE),
                  "w"):
            pass
...
def record_current_memory_usage_if_enabled() -> None:
    """
    Write the memory usage at the current context- file name and line number-
    (in bytes) to the output file.
    """
    if config.USE_MEMORY_PROFILING:
        ...
        with open(os.path.join(PROFILING_DIRECTORY, PROFILING_OUTPUT_FILE),
                  "a") as f:
            hp = guppy.hpy()
            entry = {'file_name': calling_frame.filename,
...
def generate_report_if_enabled() -> None:
    if config.USE_MEMORY_PROFILING:
        with open(os.path.join(PROFILING_DIRECTORY, PROFILING_OUTPUT_FILE),
            ...
            with open(os.path.join(PROFILING_DIRECTORY, REPORT_OUTPUT_FILE),
                      "w") as wf:
                ...
                for i, v in enumerate(raw_lines):
                    ...
                    file_name = memory_dict.get('file_name',
                                                'MISSING_FILE_NAME')
                    report_block.append(f"\tFile Name: {file_name}")
                    line_number = memory_dict.get('line_number',
                                                  'MISSING_LINE_NUMBER')
                    report_block.append(f"\tLine Number: {line_number}")
                    heap_bytes = memory_dict.get('heap_size_bytes', 0)
                    heap_mb = int(heap_bytes/1048576)
                    report_block.append(
                        f"\tCurrent Memory Usage: {heap_mb} MB")
